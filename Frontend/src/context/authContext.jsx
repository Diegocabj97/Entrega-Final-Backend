...
const AuthProvider = ({ children }) => {
  ...
  // Crea la variable initialized que indica si el contexto de autenticación ha iniciado o no. Inicialmente se establece en false
  ...
  useEffect(() => {
    const initializeToken = () => {
      // Obtiene el token almacenado en la cookie llamada "jwtCookie"
      const storedToken = getCookieValue("jwtCookie");

      // Decodifica el token y muestra la info del usuario en la consola del navegador
      try {
        if (storedToken) {
  ...
  const login = async (email, password) => {
    try {
      ...
      if (response.ok) {
        ...
        // Manejar específicamente el error de "Unauthorized"
      ...
      // Manejar el error de "Unauthorized" al iniciar sesión
      if (error.message === "Unauthorized") {
        throw new Error(
          "Credenciales no válidas. Verifique su email y contraseña."
        );
      } else {
  ...
  // token no es una función pero también se debe pasar, sino no funciona el Header
  return (
